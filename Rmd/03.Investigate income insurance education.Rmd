---
title: "Association of analgesic medication use with survival among Black women with ovarian cancer"
...
analgesics <-
  read_rds(paste0(here::here(), "/Cleaned imputed analgesics medication data_phase1_04252024.rds")) %>% 
...
### Secondary analysis - non imputed data - HR 
...
### Model 3 - + BMI_recent_grp, smokcurrent2, CCI_new_Cat + physical activity
```{r}
tbl1 <- analgesics %>% select(os_event, os_time,
                              aspirin, nsaid, aceta,
                              refage, site_2, stage, 
                              histotype2, CCI_new_Cat, 
                              BMI_recent_grp, smokcurrent2, 
                              paga, income, insurance, education) %>%
  tbl_uvregression(method = survival::coxph,
                   y = (Surv(time = analgesics$os_time,
                             event = analgesics$os_event)),
                   exponentiate = TRUE) %>% 
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + stage + strata(histotype2) +
                CCI_new_Cat + BMI_recent_grp + smokcurrent2 + paga + income + insurance + education,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
<!-- coxph(Surv(time = analgesics$os_time,  -->
<!--                    event = analgesics$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + stage + strata(histotype2) + -->
<!--                 CCI_new_Cat + BMI_recent_grp + smokcurrent2 + paga, -->
<!--               data = analgesics) %>% -->
<!--   cox.zph() -->
<!-- ``` -->
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                ...
                refage + site_2 + stage + strata(histotype2),
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin_duration_cat + nsaid_duration_cat + acetaminophen_duration_cat + 
                refage + site_2 + stage + strata(histotype2) + CCI_new_Cat,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin_duration_cat + nsaid_duration_cat + acetaminophen_duration_cat + 
                refage + site_2 + stage + strata(histotype2) + CCI_new_Cat + BMI_recent_grp + smokcurrent2 + paga + income + insurance + education,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                ...
                strata(histotype2) + NEW_dblkstat_treat_CA125,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
### Model 2 - + CCI_new_Cat + NEW_dblkstat_treat_CA125
```{r}
tbl1 <- analgesics %>% select(os_event, os_time,
                              aspirin, nsaid, aceta,
                              refage, site_2, stage, 
...
<!-- ### Model 4 - + indication + NEW_dblkstat_treat_CA125 -->
<!-- ```{r} -->
<!-- tbl1 <- analgesics %>% select(os_event, os_time, -->
<!--                               aspirin_ind, -->
<!--                               nsaid_ind,  -->
...
# VII. Multiple imputation - Primary analysis HR
...
### Model 3 - + imputed CCI_new_Cat + imp_BMI_recent_grp + smokcurrent2 + imp_paga
```{r}
tbl1 <- analgesics %>% select(os_event, os_time,
                              aspirin, nsaid, aceta,
                              refage, site_2, imp_stage, 
                              ...
                              imp_paga, imp_income, imp_insurance, imp_education) %>%
  tbl_uvregression(method = survival::coxph,
                   y = (Surv(time = analgesics$os_time,
                             event = analgesics$os_event)),
                   exponentiate = TRUE) %>% 
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage + 
                strata(imp_histotype2) +
                imp_CCI_new_Cat + imp_BMI_recent_grp + smokcurrent2 + imp_paga + imp_income + imp_insurance + imp_education,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
tbl1 <- analgesics %>% select(os_event, os_time,
                              ...
                              imp_histotype2,
                              # imp_BMI_recent_grp, smokcurrent2, 
                              imp_CCI_new_Cat#, imp_paga, imp_income, imp_insurance, imp_education
                              ) %>%
  tbl_uvregression(method = survival::coxph,
                   y = (Surv(time = analgesics$os_time,
                             event = analgesics$os_event)),
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin_duration_cat + nsaid_duration_cat + acetaminophen_duration_cat +
                refage + site_2 + imp_stage + strata(imp_histotype2) #+ imp_CCI_new_Cat + imp_income + imp_insurance + imp_education
              ,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
...
coxph(Surv(time = analgesics$os_time, 
        ...
        site_2 + imp_stage + strata(imp_histotype2) + imp_CCI_new_Cat,
      data = analgesics)  %>%
  cox.zph()
```
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin_duration_cat + nsaid_duration_cat + acetaminophen_duration_cat +
                refage + site_2 + imp_stage + strata(imp_histotype2) + imp_BMI_recent_grp + smokcurrent2 + imp_CCI_new_Cat + imp_income + imp_insurance + imp_education,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
<!-- coxph(Surv(time = analgesics$os_time,  -->
<!--            event = analgesics$os_event) ~ aspirin_duration_cat + nsaid_duration_cat + acetaminophen_duration_cat + refage + -->
<!--         site_2 + imp_stage + strata(imp_histotype2) + imp_CCI_new_Cat + imp_income + imp_insurance + imp_education, -->
<!--       data = analgesics)  %>% -->
<!--   cox.zph() -->
<!-- ``` -->
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin_freq_cat + nsaid_freq_cat + acetaminophen_freq_cat + 
                refage + site_2 + imp_stage + strata(imp_histotype2) + imp_CCI_new_Cat,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
tbl1 <- analgesics %>% select(os_event, os_time,
                              ...
                              acetaminophen_freq_cat,
                              refage, site_2, imp_stage, 
                              imp_histotype2,imp_BMI_recent_grp, smokcurrent2,
                              imp_CCI_new_Cat, imp_income, imp_insurance, imp_education) %>%
  tbl_uvregression(method = survival::coxph,
                   y = (Surv(time = analgesics$os_time,
                             event = analgesics$os_event)),
                   exponentiate = TRUE) %>% 
...
### Model 1 + imputed debulking status
...
### Model 2 - + imputed CCI_new_Cat + imputed debulking status
```{r}
tbl1 <- analgesics %>% select(os_event, os_time,
                              aspirin, nsaid, aceta,
                              refage, site_2, imp_stage, 
                              imp_histotype2, imp_CCI_new_Cat,
                              imp_debulking
                              ) %>%
  tbl_uvregression(method = survival::coxph,
                   y = (Surv(time = analgesics$os_time,
...
tbl1 <- analgesics %>% select(os_event, os_time,
                              aspirin, nsaid, aceta,
                              refage, site_2, imp_stage, 
                              imp_histotype2, imp_CCI_new_Cat, imp_BMI_recent_grp, smokcurrent2,
                              imp_debulking, imp_income, imp_insurance, imp_education
                              ) %>%
  tbl_uvregression(method = survival::coxph,
                   y = (Surv(time = analgesics$os_time,
                             event = analgesics$os_event)),
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage + 
                strata(imp_histotype2) + imp_debulking + imp_BMI_recent_grp + smokcurrent2 +
                imp_CCI_new_Cat + imp_income + imp_insurance + imp_education,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
...
<!-- # VIII. Stratified by histotype using multiple imputation data -->
<!-- ## HGSC/carcinosarcoma -->
<!-- ```{r} -->
...
<!-- ### 1. Yes/No -->
<!-- #### Model 1 - adjust for age, study site, imputed stage -->
<!-- ```{r} -->
<!-- tbl1 <- analgenics_HGSC %>% select(os_event, os_time, -->
<!--                               aspirin, nsaid, aceta, -->
<!--                               refage, site_2, imp_stage -->
<!--                               ) %>% -->
<!--   tbl_uvregression(method = survival::coxph, -->
<!--                    y = (Surv(time = analgenics_HGSC$os_time, -->
<!--                              event = analgenics_HGSC$os_event)), -->
<!--                    exponentiate = TRUE) %>%  -->
<!--   bold_labels() %>% italicize_levels() %>%  -->
<!--   bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level") -->

<!-- tbl2 <- coxph(Surv(time = analgenics_HGSC$os_time,  -->
...
<!-- tbl2 <- coxph(Surv(time = analgenics_HGSC$os_time,  -->
<!--                    event = analgenics_HGSC$os_event) ~ aspirin_duration_cat + nsaid_duration_cat + acetaminophen_duration_cat + -->
<!--                 refage + site_2 + imp_stage + imp_CCI_new_Cat, -->
<!--               data = analgenics_HGSC)  %>%  -->
<!--   tbl_regression(exponentiate = TRUE) %>%  -->
<!--   bold_p(t = .05) %>%  -->
<!--   add_nevent(location = "level") %>% add_n(location = "level") -->
...
<!-- #### Model 1 - adjust for age, study site, imputed stage -->
<!-- ```{r} -->
<!-- tbl1 <- analgenics_notHGSC %>% select(os_event, os_time, -->
<!--                               aspirin, nsaid, aceta, -->
<!--                               refage, site_2, imp_stage -->
<!--                               ) %>% -->
<!--   tbl_uvregression(method = survival::coxph, -->
<!--                    y = (Surv(time = analgenics_notHGSC$os_time, -->
<!--                              event = analgenics_notHGSC$os_event)), -->
<!--                    exponentiate = TRUE) %>%  -->
<!--   bold_labels() %>% italicize_levels() %>%  -->
<!--   bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level") -->

<!-- tbl2 <- coxph(Surv(time = analgenics_notHGSC$os_time,  -->
...
<!-- # IX. Stratified by CCI using multiple imputation data -->
<!-- # CCI score is 0 -->
<!-- ```{r} -->
...
<!-- #### Model 1 - adjust for age, study site, imputed stage, and imputed histotype -->
<!-- ```{r} -->
<!-- tbl1 <- analgenics_CCI_0 %>% select(os_event, os_time, -->
<!--                               aspirin, nsaid, aceta, -->
<!--                               refage, site_2, imp_stage,  -->
<!--                               imp_histotype2) %>% -->
<!--   tbl_uvregression(method = survival::coxph, -->
<!--                    y = (Surv(time = analgenics_CCI_0$os_time, -->
<!--                              event = analgenics_CCI_0$os_event)), -->
<!--                    exponentiate = TRUE) %>%  -->
<!--   bold_labels() %>% italicize_levels() %>%  -->
<!--   bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level") -->

<!-- tbl2 <- coxph(Surv(time = analgenics_CCI_0$os_time,  -->
...
<!-- PH assumption -->
<!-- ```{r} -->
<!-- coxph(Surv(time = analgenics_CCI_0$os_time,  -->
<!--                    event = analgenics_CCI_0$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage +  -->
<!--         strata(imp_histotype2), -->
<!--               data = analgenics_CCI_0)  %>% -->
<!--   cox.zph() -->
...
<!-- #### Model 3 - + imp_BMI_recent_grp + smokcurrent2 + imp_paga -->
<!-- ```{r} -->
<!-- tbl1 <- analgenics_CCI_0 %>% select(os_event, os_time, -->
<!--                               aspirin, nsaid, aceta, -->
<!--                               refage, site_2, imp_stage,  -->
...
<!--                    event = analgenics_CCI_0$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage +  -->
<!--                 strata(imp_histotype2) + -->
<!--                 imp_BMI_recent_grp + smokcurrent2 + imp_paga + imp_income + imp_insurance + imp_education, -->
<!--               data = analgenics_CCI_0)  %>%  -->
<!--   tbl_regression(exponentiate = TRUE) %>%  -->
<!--   bold_p(t = .05) %>%  -->
<!--   add_nevent(location = "level") %>% add_n(location = "level") -->
...
<!--                    event = analgenics_CCI_0$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage +  -->
<!--         strata(imp_histotype2) + -->
<!--                 imp_BMI_recent_grp + smokcurrent2 + imp_paga, -->
<!--               data = analgenics_CCI_0) %>% -->
<!--   cox.zph() -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- coxph(Surv(time = analgenics_CCI_0$os_time,  -->
<!--            event = analgenics_CCI_0$os_event) ~ aspirin_duration_cat + nsaid_duration_cat + acetaminophen_duration_cat + refage + -->
<!--         site_2 + imp_stage + strata(imp_histotype2), -->
<!--       data = analgenics_CCI_0)  %>% -->
<!--   cox.zph() -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- coxph(Surv(time = analgenics_CCI_0$os_time,  -->
<!--            event = analgenics_CCI_0$os_event) ~ aspirin_freq_cat + nsaid_freq_cat + acetaminophen_freq_cat + refage + -->
<!--         site_2 + imp_stage + strata(imp_histotype2), -->
<!--       data = analgenics_CCI_0)  %>% -->
<!--   cox.zph() -->
<!-- ``` -->
...
<!-- #### Model + imputed debulking status -->
<!-- ```{r} -->
<!-- tbl1 <- analgenics_CCI_0 %>% select(os_event, os_time, -->
<!--                               aspirin, nsaid, aceta, -->
<!--                               refage, site_2, imp_stage,  -->
...
<!-- # CCI score is 1 -->
<!-- ```{r} -->
<!-- analgenics_CCI_1 <- analgesics %>%  -->
<!--   filter(imp_CCI_new_Cat == "CCI score is 1") -->
<!-- ``` -->
...
<!-- #### Model 1 - adjust for age, study site, imputed stage, and imputed histotype -->
<!-- ```{r} -->
<!-- tbl1 <- analgenics_CCI_1 %>% select(os_event, os_time, -->
<!--                               aspirin, nsaid, aceta, -->
<!--                               refage, site_2, imp_stage,  -->
<!--                               imp_histotype2) %>% -->
<!--   tbl_uvregression(method = survival::coxph, -->
<!--                    y = (Surv(time = analgenics_CCI_1$os_time, -->
<!--                              event = analgenics_CCI_1$os_event)), -->
<!--                    exponentiate = TRUE) %>%  -->
<!--   bold_labels() %>% italicize_levels() %>%  -->
<!--   bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level") -->

<!-- tbl2 <- coxph(Surv(time = analgenics_CCI_1$os_time,  -->
<!--                    event = analgenics_CCI_1$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage +  -->
<!--                 strata(imp_histotype2), -->
<!--               data = analgenics_CCI_1)  %>%  -->
<!--   tbl_regression(exponentiate = TRUE) %>%  -->
<!--   bold_p(t = .05) %>%  -->
<!--   add_nevent(location = "level") %>% add_n(location = "level") -->
<!-- tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**")) -->
...
<!-- PH assumption -->
<!-- ```{r} -->
<!-- coxph(Surv(time = analgenics_CCI_1$os_time,  -->
<!--                    event = analgenics_CCI_1$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage +  -->
<!--         strata(imp_histotype2), -->
<!--               data = analgenics_CCI_1)  %>% -->
<!--   cox.zph() -->
...
<!-- tbl2 <- coxph(Surv(time = analgenics_CCI_1$os_time,  -->
<!--                    event = analgenics_CCI_1$os_event) ~ aspirin_duration_cat + nsaid_duration_cat + acetaminophen_duration_cat +  -->
<!--                 refage + site_2 + imp_stage + strata(imp_histotype2), -->
<!--               data = analgenics_CCI_1)  %>%  -->
<!--   tbl_regression(exponentiate = TRUE) %>%  -->
<!--   bold_p(t = .05) %>%  -->
...
<!-- ```{r} -->
<!-- coxph(Surv(time = analgenics_CCI_1$os_time,  -->
<!--            event = analgenics_CCI_1$os_event) ~ aspirin_duration_cat + nsaid_duration_cat + acetaminophen_duration_cat + refage + -->
<!--         site_2 + imp_stage + strata(imp_histotype2), -->
<!--       data = analgenics_CCI_1)  %>% -->
<!--   cox.zph() -->
<!-- ``` -->
...
<!-- tbl2 <- coxph(Surv(time = analgenics_CCI_1$os_time,  -->
<!--                    event = analgenics_CCI_1$os_event) ~ aspirin_freq_cat + nsaid_freq_cat + acetaminophen_freq_cat +  -->
<!--                 refage + site_2 + imp_stage + strata(imp_histotype2), -->
<!--               data = analgenics_CCI_1)  %>%  -->
<!--   tbl_regression(exponentiate = TRUE) %>%  -->
<!--   bold_p(t = .05) %>%  -->
...
<!-- ```{r} -->
<!-- coxph(Surv(time = analgenics_CCI_1$os_time,  -->
<!--            event = analgenics_CCI_1$os_event) ~ aspirin_freq_cat + nsaid_freq_cat + acetaminophen_freq_cat + refage + -->
<!--         site_2 + imp_stage + strata(imp_histotype2), -->
<!--       data = analgenics_CCI_1)  %>% -->
<!--   cox.zph() -->
<!-- ``` -->
...
<!-- # CCI score is 2 -->
<!-- ```{r} -->
<!-- analgenics_CCI_2 <- analgesics %>%  -->
<!--   filter(imp_CCI_new_Cat == "CCI score is 2+") -->
<!-- ``` -->
...
<!-- #### Model 1 - adjust for age, study site, imputed stage, and imputed histotype -->
<!-- ```{r} -->
<!-- tbl1 <- analgenics_CCI_2 %>% select(os_event, os_time, -->
<!--                               aspirin, nsaid, aceta, -->
<!--                               refage, site_2, imp_stage,  -->
<!--                               imp_histotype2) %>% -->
<!--   tbl_uvregression(method = survival::coxph, -->
<!--                    y = (Surv(time = analgenics_CCI_2$os_time, -->
<!--                              event = analgenics_CCI_2$os_event)), -->
<!--                    exponentiate = TRUE) %>%  -->
<!--   bold_labels() %>% italicize_levels() %>%  -->
<!--   bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level") -->

<!-- tbl2 <- coxph(Surv(time = analgenics_CCI_2$os_time,  -->
<!--                    event = analgenics_CCI_2$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage +  -->
<!--                 strata(imp_histotype2), -->
<!--               data = analgenics_CCI_2)  %>%  -->
<!--   tbl_regression(exponentiate = TRUE) %>%  -->
<!--   bold_p(t = .05) %>%  -->
<!--   add_nevent(location = "level") %>% add_n(location = "level") -->
<!-- tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**")) -->
...
<!-- PH assumption -->
<!-- ```{r} -->
<!-- coxph(Surv(time = analgenics_CCI_2$os_time,  -->
<!--                    event = analgenics_CCI_2$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage +  -->
<!--         strata(imp_histotype2), -->
<!--               data = analgenics_CCI_2)  %>% -->
<!--   cox.zph() -->
...
<!-- tbl2 <- coxph(Surv(time = analgenics_CCI_2$os_time,  -->
<!--                    event = analgenics_CCI_2$os_event) ~ aspirin_duration_cat + nsaid_duration_cat + acetaminophen_duration_cat +  -->
<!--                 refage + site_2 + imp_stage + strata(imp_histotype2), -->
<!--               data = analgenics_CCI_2)  %>%  -->
<!--   tbl_regression(exponentiate = TRUE) %>%  -->
<!--   bold_p(t = .05) %>%  -->
...
<!-- ```{r} -->
<!-- coxph(Surv(time = analgenics_CCI_2$os_time,  -->
<!--            event = analgenics_CCI_2$os_event) ~ aspirin_duration_cat + nsaid_duration_cat + acetaminophen_duration_cat + refage + -->
<!--         site_2 + imp_stage + strata(imp_histotype2), -->
<!--       data = analgenics_CCI_2)  %>% -->
<!--   cox.zph() -->
<!-- ``` -->
...
<!-- tbl2 <- coxph(Surv(time = analgenics_CCI_2$os_time,  -->
<!--                    event = analgenics_CCI_2$os_event) ~ aspirin_freq_cat + nsaid_freq_cat + acetaminophen_freq_cat +  -->
<!--                 refage + site_2 + imp_stage + strata(imp_histotype2), -->
<!--               data = analgenics_CCI_2)  %>%  -->
<!--   tbl_regression(exponentiate = TRUE) %>%  -->
<!--   bold_p(t = .05) %>%  -->
...
<!-- ```{r} -->
<!-- coxph(Surv(time = analgenics_CCI_2$os_time,  -->
<!--            event = analgenics_CCI_2$os_event) ~ aspirin_freq_cat + nsaid_freq_cat + acetaminophen_freq_cat + refage + -->
<!--         site_2 + imp_stage + strata(imp_histotype2), -->
<!--       data = analgenics_CCI_2)  %>% -->
<!--   cox.zph() -->
<!-- ``` -->
