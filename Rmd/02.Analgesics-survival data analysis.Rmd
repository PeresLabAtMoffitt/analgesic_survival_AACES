...
analgesics <-
  read_rds(paste0(here::here(), "/Cleaned analgesics medication data_03062024.rds"))
```
...
analgesics %>% 
  select(refage, stage, histotype, histotype2,
         ...
         neoadj_treat, NEW_dblkstat_treat_CA125, 
         CCI_new, CCI_new_Cat) %>% 
...
analgesics %>% 
  select(aspirin, starts_with("aspirin_duration"), 
         starts_with("aspirin_freq"), aspirin_ind,
         nsaid, starts_with("nsaid_duration"), 
         starts_with("nsaid_freq"), nsaid_ind,
         aceta, starts_with("acetaminophen_duration"),
         starts_with("acetaminophen_freq"), aceta_ind) %>% 
  tbl_summary(type = list(
...
analgesics %>% 
  select(refage, stage, histotype, histotype2,
         BMI_recent, bmicat_2, menopause, smokcurrent2, 
         neoadj_treat, NEW_dblkstat_treat_CA125, NEW_dblkstat_treat_CA125, 
         CCI_new, CCI_new_Cat,
         aspirin, nsaid, aceta) %>% 
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin + aspirin_ind + nsaid + nsaid_ind + aceta + aceta_ind + refage + site_2 + stage + histotype2,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
```{r}
coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin + aspirin_ind + nsaid + aceta + refage + site_2 + stage + histotype2,
              data = analgesics)  %>%
  cox.zph()
```


# V. Using a cut point - to come


# VI. Sensitivity analysis adjusting for treatment 
...
### Model 2 - + bmicat_2, smokcurrent2, CCI_new_Cat (could use continuous + need to ask Alicia) - I don't have physical activity + NEW_dblkstat_treat_CA125
```{r}
tbl1 <- analgesics %>% select(os_event, os_time,
                              aspirin, nsaid, aceta,
                              refage, site_2, stage, 
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin + aspirin_ind + nsaid + nsaid_ind + aceta + aceta_ind + refage + site_2 + stage + histotype2 + NEW_dblkstat_treat_CA125,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
```{r}
coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin + aspirin_ind + nsaid + aceta + refage + site_2 + stage + histotype2 + NEW_dblkstat_treat_CA125,
              data = analgesics)  %>%
  cox.zph()
```
