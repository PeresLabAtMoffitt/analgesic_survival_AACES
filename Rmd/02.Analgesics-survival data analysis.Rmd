...
analgesics <-
  read_rds(paste0(here::here(), "/Cleaned imputed analgesics medication data_03132024.rds"))
```
...
var_label(analgesics) <- list(refage = "Patient age", 
                         ...
                         BMI_recent = "BMI (recent)", bmicat_2 = "BMI categories", 
...
```{r Clinical table}
analgesics %>% 
  select(refage, stage, imp_stage, histotype, histotype2,
         BMI_recent, bmicat_2, menopause, smokcurrent2, 
         neoadj_treat, NEW_dblkstat_treat_CA125, imp_debulking,
         CCI_new, CCI_new_Cat) %>% 
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + stage + histotype2,
              data = analgesics)  %>% 
...
### Model 2 - + bmicat_2, smokcurrent2, CCI_new_Cat - I don't have physical activity
```{r}
tbl1 <- analgesics %>% select(os_event, os_time,
                              aspirin, nsaid, aceta,
                              refage, site_2, stage, 
                              histotype2, 
                              bmicat_2,smokcurrent2, 
                              CCI_new_Cat) %>%
  tbl_uvregression(method = survival::coxph,
                   y = (Surv(time = analgesics$os_time,
                             event = analgesics$os_event)),
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin_ind + nsaid_ind + aceta_ind + refage + site_2 + stage + histotype2,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin_duration_cat + nsaid_duration_cat + acetaminophen_duration_cat + refage + site_2 + stage + histotype2,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin_freq_cat + nsaid_freq_cat + acetaminophen_freq_cat + refage + site_2 + stage + histotype2,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + stage + histotype2 + NEW_dblkstat_treat_CA125,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
### Model 2 - + bmicat_2, smokcurrent2, CCI_new_Cat - I don't have physical activity + NEW_dblkstat_treat_CA125
```{r}
tbl1 <- analgesics %>% select(os_event, os_time,
                              aspirin, nsaid, aceta,
                              refage, site_2, stage, 
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin_ind + nsaid_ind + aceta_ind + refage + site_2 + stage + histotype2 + NEW_dblkstat_treat_CA125,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
## 1. Yes/No
### Model 1 - adjust for age, study site, imp_stage, and histotype
```{r}
tbl1 <- analgesics %>% select(os_event, os_time,
                              aspirin, nsaid, aceta,
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage + histotype2,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
### Model 2 - + bmicat_2, smokcurrent2, CCI_new_Cat (could use continuous) - I don't have physical activity
```{r}
tbl1 <- analgesics %>% select(os_event, os_time,
                              aspirin, nsaid, aceta,
                              refage, site_2, imp_stage, 
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin_ind + nsaid_ind + aceta_ind + refage + site_2 + imp_stage + histotype2,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin_duration_cat + nsaid_duration_cat + acetaminophen_duration_cat + refage + site_2 + imp_stage + histotype2,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin_freq_cat + nsaid_freq_cat + acetaminophen_freq_cat + refage + site_2 + imp_stage + histotype2,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage + histotype2 + imp_debulking,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
...
### Model 2 - + bmicat_2, smokcurrent2, CCI_new_Cat (could use continuous + need to ask Alicia) - I don't have physical activity + imp_debulking
```{r}
tbl1 <- analgesics %>% select(os_event, os_time,
                              aspirin, nsaid, aceta,
                              refage, site_2, imp_stage, 
...
tbl2 <- coxph(Surv(time = analgesics$os_time, 
                   event = analgesics$os_event) ~ aspirin_ind + nsaid_ind + aceta_ind + refage + site_2 + imp_stage + histotype2 + imp_debulking,
              data = analgesics)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
