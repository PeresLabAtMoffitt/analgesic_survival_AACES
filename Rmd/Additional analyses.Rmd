...
```{r}
analgesics_HGSC <- analgesics %>%
  filter(imp_histotype2 == "HGSC/carcinosarcoma")
```

### a. Yes/No users
```{r}
model_1 <- coxph(Surv(time = analgesics_HGSC$os_time, 
                   event = analgesics_HGSC$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage,
              data = analgesics_HGSC)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_labels() %>% italicize_levels() %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")

model_2 <- coxph(Surv(time = analgesics_HGSC$os_time, 
                      event = analgesics_HGSC$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage + 
...
model_3 <- coxph(Surv(time = analgesics_HGSC$os_time, 
                   event = analgesics_HGSC$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage + 
                  imp_CCI_new_Cat + imp_BMI_recent_grp + imp_private_insurance + imp_medicare_insurance +
                   imp_medicaid_insurance + 
                  imp_debulking,
...
```{r}
model_1 <- coxph(Surv(time = analgesics_HGSC$os_time, 
                   event = analgesics_HGSC$os_event) ~ aspirin_duration_cat + nsaid_duration_cat + acetaminophen_duration_cat + 
...
```{r}
model_1 <- coxph(Surv(time = analgesics_HGSC$os_time, 
                   event = analgesics_HGSC$os_event) ~ aspirin_freq_cat + nsaid_freq_cat + acetaminophen_freq_cat + 
                refage + site_2 + imp_stage,
              data = analgesics_HGSC)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_labels() %>% italicize_levels() %>% 
...
```{r}
analgesics_notHGSC <- analgesics %>%
  filter(imp_histotype2 == "not HGSC/carcinosarcoma")
```

### a. Yes/No users
```{r}
model_1 <- coxph(Surv(time = analgesics_notHGSC$os_time, 
                   event = analgesics_notHGSC$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage,
              data = analgesics_notHGSC)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_labels() %>% italicize_levels() %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")

model_2 <- coxph(Surv(time = analgesics_notHGSC$os_time, 
                      event = analgesics_notHGSC$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage + 
                   imp_CCI_new_Cat + imp_BMI_recent_grp + imp_private_insurance + imp_medicare_insurance +
                   imp_medicaid_insurance,
                 data = analgesics_notHGSC)  %>% 
...
model_3 <- coxph(Surv(time = analgesics_notHGSC$os_time, 
                   event = analgesics_notHGSC$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage + 
                  imp_CCI_new_Cat + imp_BMI_recent_grp + imp_private_insurance + imp_medicare_insurance +
                   imp_medicaid_insurance + 
                  imp_debulking,
...
```{r}
model_1 <- coxph(Surv(time = analgesics_notHGSC$os_time, 
                   event = analgesics_notHGSC$os_event) ~ aspirin_duration_cat + nsaid_duration_cat + acetaminophen_duration_cat + 
                refage + site_2 + imp_stage,
              data = analgesics_notHGSC)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_labels() %>% italicize_levels() %>% 
...
```{r}
# model_1 <- coxph(Surv(time = analgesics_notHGSC$os_time, 
#                    event = analgesics_notHGSC$os_event) ~ aspirin_freq_cat + nsaid_freq_cat + acetaminophen_freq_cat + 
#                 refage + site_2 + imp_stage,
#               data = analgesics_notHGSC)  %>% 
#   tbl_regression(exponentiate = TRUE) %>% 
#   bold_labels() %>% italicize_levels() %>% 
...
## 1. CCI score is 0
```{r}
analgesics_CCI_0 <- analgesics %>%
  filter(imp_CCI_new_Cat == "CCI score is 0")
```

### a. Yes/No users
```{r}
model_1 <- coxph(Surv(time = analgesics_CCI_0$os_time, 
                   event = analgesics_CCI_0$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage + 
                strata(imp_histotype2),
              data = analgesics_CCI_0)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_labels() %>% italicize_levels() %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")

model_2 <- coxph(Surv(time = analgesics_CCI_0$os_time, 
                      event = analgesics_CCI_0$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage + 
                strata(imp_histotype2) + 
...
model_3 <- coxph(Surv(time = analgesics_CCI_0$os_time, 
                   event = analgesics_CCI_0$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage + 
                strata(imp_histotype2) + 
                  imp_BMI_recent_grp + imp_private_insurance + imp_medicare_insurance +
                   imp_medicaid_insurance + 
...
```{r}
model_1 <- coxph(Surv(time = analgesics_CCI_0$os_time, 
                   event = analgesics_CCI_0$os_event) ~ aspirin_duration_cat + nsaid_duration_cat + acetaminophen_duration_cat + 
                refage + site_2 + imp_stage + 
                strata(imp_histotype2),
              data = analgesics_CCI_0)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
...
```{r}
model_1 <- coxph(Surv(time = analgesics_CCI_0$os_time, 
                   event = analgesics_CCI_0$os_event) ~ aspirin_freq_cat + nsaid_freq_cat + acetaminophen_freq_cat + 
                refage + site_2 + imp_stage + 
                strata(imp_histotype2),
              data = analgesics_CCI_0)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
...
## 1. CCI score ≥ 1
```{r}
analgesics_CCI_other <- analgesics %>%
  filter(imp_CCI_new_Cat != "CCI score is 0")
```

### a. Yes/No users
```{r}
model_1 <- coxph(Surv(time = analgesics_CCI_other$os_time, 
                   event = analgesics_CCI_other$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage + 
                strata(imp_histotype2),
              data = analgesics_CCI_other)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
  bold_labels() %>% italicize_levels() %>% 
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")

model_2 <- coxph(Surv(time = analgesics_CCI_other$os_time, 
                      event = analgesics_CCI_other$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage + 
                strata(imp_histotype2) + 
                   imp_BMI_recent_grp + imp_private_insurance + imp_medicare_insurance +
                   imp_medicaid_insurance,
...
model_3 <- coxph(Surv(time = analgesics_CCI_other$os_time, 
                   event = analgesics_CCI_other$os_event) ~ aspirin + nsaid + aceta + refage + site_2 + imp_stage + 
                strata(imp_histotype2) + 
                  imp_BMI_recent_grp + imp_private_insurance + imp_medicare_insurance +
                   imp_medicaid_insurance + 
...
```{r}
model_1 <- coxph(Surv(time = analgesics_CCI_other$os_time, 
                   event = analgesics_CCI_other$os_event) ~ aspirin_duration_cat + nsaid_duration_cat + acetaminophen_duration_cat + 
                refage + site_2 + imp_stage + 
                strata(imp_histotype2),
              data = analgesics_CCI_other)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
...
```{r}
model_1 <- coxph(Surv(time = analgesics_CCI_other$os_time, 
                   event = analgesics_CCI_other$os_event) ~ aspirin_freq_cat + nsaid_freq_cat + acetaminophen_freq_cat + 
                refage + site_2 + imp_stage + 
                strata(imp_histotype2),
              data = analgesics_CCI_other)  %>% 
  tbl_regression(exponentiate = TRUE) %>% 
